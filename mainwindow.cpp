...
void MainWindow::setupSimpleDemo(QCustomPlot *customPlot)
{
  ...
  QVector<double> x(251), y0(251), y1(251), yn(251), y3(251);
  ...
  {
    x[i] = i;
    y0[i] = qSin(2*M_PI*150*i/7000.0)+qSin(2*M_PI*450*i/6000.0);//qExp(-i/150.0)*qCos(i/10.0); // exponentially decaying cosine
//    y1[i] = 1.0;//qExp(-i/150.0);              // exponential envelope
  }
  // calculate filter
  for (int i=0; i<251-20; i++){ //-20 karena b nya ada 20
      yn[i]=0;
      for (int j=0; j < 20; j++)
          yn[i] += y0[i+j] * bfac[j]; //yn =output, y0 =input.
  }
  rn.resize(251);
  for (int i=0; i<251; ++i)
  {
  ...
  customPlot->graph(0)->setData(x, y0); //input plot
  customPlot->graph(1)->setData(x, y1); //output plot
  ...
  ui->customPlot_2->addGraph();
  ui->customPlot_2->graph(1)->setPen(QPen(Qt::red)); // line color red for second graph
  // generate some points of data (y0 for first, y1 for second graph):
...
//  Sk = fftr2c( rn );
  ...
  // pass data points to graphs:
  ui->customPlot_2->graph(0)->setData(x, y3); //input plot
//  ui->customPlot_2->graph(1)->setData(x, y1); //output plot
  // let the ranges scale themselves so graph 0 fits perfectly in the visible area:
...
void MainWindow::on_FFTButton_clicked()
{
    ...
    for( int len=MINLEN; len<MAXLEN; len+=STEP )
    {
        ...
        Sk = fftc2c( sn ); //time to frequency domain
        xn = ifftc2c( Sk ); //frequency to time domain
//        Sk = fft( sn );
//        xn = ifft( Sk );
